...
\Input{\texttt{event\_defs}, \texttt{ltl\_exprs}, \texttt{persona\_def}, \texttt{output\_file}}
\Output{Synthetic dataset saved to \texttt{output\_file}}
$N \gets \texttt{persona\_def.sample\_size}$\;
$D \gets \texttt{persona\_def.horizon.weeks} \times 7$\;
Initialize empty list \texttt{personas}\;
\For{$i\gets1$ \KwTo $N$}{
    Generate persona attributes (e.g., age, gender)\;
    Add persona to \texttt{personas}\;
}
Initialize empty list \texttt{all\_data}\;
\For{$id\gets1$ \KwTo $N$}{
    \texttt{persona} $\gets \texttt{personas}[id]$\;
    Initialize \texttt{person\_data} with \texttt{persona}\;
    \texttt{event\_counts} $\gets$ \texttt{compute\_event\_counts}(\\
        \hspace{15pt}  \texttt{event\_defs}, $D$, $id$, \texttt{persona\_def}\\
        \hspace{5pt} )\; 
    \texttt{spillovers} $\gets$ None\;
    \For{$day\gets1$ \KwTo $D$}{
        \texttt{counts} $\gets \texttt{event\_counts}[day]$\;
        (\texttt{solver}, \texttt{event\_vars}) $\gets$\\
        \hspace{8pt}\texttt{build\_z3\_model\_for\_day}(\\
            \hspace{15pt} $day$, $D$, \texttt{event\_defs}, \texttt{ltl\_exprs},\\
            \hspace{15pt} \texttt{spillovers}, \texttt{counts},\\
            \hspace{15pt} \texttt{persona\_def}\\
        \hspace{8pt})\;
      \Indp
        \Indm
        \If{solver is satisfiable}{
            \texttt{model} $\gets \texttt{solver.model()}$\;
            \texttt{spillovers} $\gets$ \\
            \hspace{8pt}\texttt{extract\_spillovers}(\\
            \hspace{15pt}\texttt{model}, \texttt{event\_vars}\\
            \hspace{8pt})\;
            Append events from \texttt{model} to \texttt{person\_data}\;
        }
        \Else{
            Append UNSAT marker to \texttt{person\_data}\;
        }
    }
    Append \texttt{person\_data} to \texttt{all\_data}\;
}
Save \texttt{all\_data} to \texttt{output\_file}\;
